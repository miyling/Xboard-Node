...
.PHONY: build clean test docker install build-linux build-linux-arm64 build-windows build-windows-arm64 build-all
...
# Build for Windows Server amd64
build-windows:
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -tags "with_quic with_utls with_wireguard with_acme with_clash_api" -o xboard-node-windows-amd64.exe ./cmd/xboard-node
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o xbctl-windows-amd64.exe ./cmd/xbctl

# Build for Windows Server arm64
build-windows-arm64:
	CGO_ENABLED=0 GOOS=windows GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -tags "with_quic with_utls with_wireguard with_acme with_clash_api" -o xboard-node-windows-arm64.exe ./cmd/xboard-node
	CGO_ENABLED=0 GOOS=windows GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o xbctl-windows-arm64.exe ./cmd/xbctl
...
clean:
	rm -f xboard-node xbctl xboard-node-linux-* xbctl-linux-* xboard-node-windows-*.exe xbctl-windows-*.exe
